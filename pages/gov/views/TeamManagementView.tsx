...
const TeamManagementView: React.FC<TeamManagementViewProps> = ({ team, issues, isLoading, onAddMember, onUpdateMember, onDeleteMember }) => {
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editingMember, setEditingMember] = useState<TeamMember | null>(null);

    const teamStats = useMemo(() => {
        const statsMap: Record<string, CalculatedStats> = {};

        team.forEach(member => {
            const memberIssues = issues.filter(issue => issue.assigned_to?.name === member.name);
            const assigned = memberIssues.filter(i => i.status !== ReportStatus.Resolved).length;
            const resolved = memberIssues.filter(i => i.status === ReportStatus.Resolved).length;
            
            // Dummy calculation for avgTime and sla for now
            const avgTime = `${(Math.random() * 10 + 2).toFixed(1)}h`;
            const sla = 90 + Math.floor(Math.random() * 10);

            statsMap[member.id] = { assigned, resolved, avgTime, sla };
        });
                ...
                {isLoading ? (
                    ...
                    team.map(member => (
                        <TeamMemberCard 
                            key={member.id} 
                            member={member}
                            stats={teamStats[member.id] || { assigned: 0, resolved: 0, avgTime: 'N/A', sla: 0 }}
